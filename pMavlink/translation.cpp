    ...
    case MAVLINK_MSG_ID_GPS_RAW_INT:
    	{
            ...
            uint16_t ground_speed = mavlink_msg_gps_raw_int_get_vel(msg);
            Notify("NAV_SPEED", ground_speed / 100.0);
...
//    cout<<key<<"inside "<<endl;


    if (key == "DESIRED_SPEED"|| key == "DESIRED_HEADING") {

        if(key == "DESIRED_SPEED"){
            m_desired_speed = (float) msg.GetDouble();
        }

        if(key == "DESIRED_HEADING"){
            m_desired_heading_indeg = (float)msg.GetDouble();
        }

        m_desired_heading_inrad = (float) m_desired_heading_indeg * (PI/180.0);
        vy = (float) m_desired_speed * sin (m_desired_heading_inrad);
        vx = (float) m_desired_speed * cos (m_desired_heading_inrad);

        yaw = (float) m_desired_heading_inrad;
        yaw_rate = 0.2618;

        float x   = 0.0;
        float y   = 0.0;
        float z   = -m_desired_altitude;
        ...
        uint16_t type_mask = MAVLINK_MSG_SET_POSITION_TARGET_LOCAL_NED_CUSTOM;
